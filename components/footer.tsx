    ...
    <footer className="relative z-10 border-t border-white/10 bg-gray-900/80">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-14">
        <div className="grid gap-10 md:grid-cols-2 lg:grid-cols-4">
          ...
          <div className="space-y-4">
            <h3 className="font-mono text-[13px] sm:text-xs font-bold uppercase tracking-widest text-white">
              Leistungen
            </h3>
            <ul className="space-y-1 text-sm text-gray-400 md:space-y-2">
              {b2bServices.map((service) => (
                <li key={service.slug}>
                  <Link
                    href={`/leistungen/${service.slug}`}
                    className="inline-block py-3 transition-colors hover:text-brass md:py-0.5"
                  >
                    {service.navLabel}
                  </Link>
                </li>
          ...
          <div className="space-y-4">
            ...
            <ul className="space-y-3 text-sm text-gray-400">
              <li>
                <a
                  href={mailtoLink}
                  className="inline-flex items-center gap-2 py-3 transition-colors hover:text-brass md:py-0.5"
                >
            ...
            <Link
              href={contactHref}
              className="inline-flex min-h-11 items-center rounded-sm border border-brass/40 px-4 py-3 font-mono text-[12px] sm:text-[11px] uppercase tracking-wider text-brass transition-colors hover:bg-brass/10"
            >
              Fall schildern
            </Link>
          </div>
        ...
        <div className="mt-12 flex flex-col items-center justify-between gap-4 border-t border-white/10 pt-8 md:flex-row">
          <p className="font-mono text-[13px] sm:text-xs text-gray-400">
            © {new Date().getFullYear()} BONA FIDES Detektei. Alle Rechte vorbehalten.
          </p>
          <div className="flex flex-wrap justify-center gap-x-6 gap-y-2 font-mono text-[13px] sm:text-xs text-gray-400">
            {legalLinks.map((link) => (
              <Link key={link.href} href={link.href} className="inline-block py-3 transition-colors hover:text-brass md:py-0.5">
                {link.label}
              </Link>
            ))}
            <button onClick={reopenBanner} className="cursor-pointer py-3 transition-colors hover:text-brass md:py-0.5">
              Cookies
            </button>
          </div>
        </div>
