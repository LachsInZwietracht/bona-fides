#!/usr/bin/env python3
"""
Digitale Spur Content Similarity Analysis
Analyzes 20 numbered Digitale Spur articles for content duplicates
...
class DigitaleSpurAnalyzer:
    ...
    def analyze_article(self, filepath: Path) -> Dict:
        ...
        try:
            ...
            number_match = re.search(r'digitale-spur-(\d+)-', filepath.name)
            article_number = int(number_match.group(1)) if number_match else 0

            return {
                'filepath': str(filepath),
                'article_number': article_number,
                'title': f"Digitale Spur {article_number}" if article_number else filepath.stem,
                'sections': sections,
    ...
    def load_all_articles(self) -> Dict[str, Dict]:
        """Load and analyze all Digitale Spur articles"""
        digitale_spur_files = sorted(self.articles_dir.glob("*digitale-spur*moderne-ermittlungstechniken-im-netz.md"))
        print(f"Found {len(digitale_spur_files)} Digitale Spur articles")

        for filepath in digitale_spur_files:
    ...
    def generate_report(self, similarity_matrix: Dict, deletion_candidates: List[Dict]) -> str:
        ...
        report = []
        report.append("# Digitale Spur Content Similarity Analysis Report")
...
def main():
    ...
    analyzer = DigitaleSpurAnalyzer(articles_dir)

    # Load all articles
    print("Loading and analyzing Digitale Spur articles...")
    ...
    # Save report
    report_path = Path("/Users/fabianradlow/Documents/Code-Projekte/Operation Bona Fides/digitale_spur_analysis_report.md")
