#!/usr/bin/env python3
"""
Branchenstatistik Content Similarity Analysis
Analyzes 10 numbered Branchenstatistik articles for content duplicates
...
class BranchenstatistikAnalyzer:
    ...
    def analyze_article(self, filepath: Path) -> Dict:
        ...
        try:
            ...
            if 'branchenstatistik-1-' in filepath.name:
                article_number = 1
            else:
                number_match = re.search(r'branchenstatistik-(\d+)-', filepath.name)
                article_number = int(number_match.group(1)) if number_match else 0

            return {
                'filepath': str(filepath),
                'article_number': article_number,
                'title': f"Branchenstatistik {article_number}" if article_number else filepath.stem,
                'sections': sections,
    ...
    def load_all_articles(self) -> Dict[str, Dict]:
        """Load and analyze all Branchenstatistik articles"""
        branchenstatistik_files = sorted(self.articles_dir.glob("*branchenstatistik*aktuelle-trends-im-cybercrime.md"))
        print(f"Found {len(branchenstatistik_files)} Branchenstatistik articles")

        for filepath in branchenstatistik_files:
    ...
    def generate_report(self, similarity_matrix: Dict, deletion_candidates: List[Dict]) -> str:
        ...
        report = []
        report.append("# Branchenstatistik Content Similarity Analysis Report")
...
def main():
    ...
    analyzer = BranchenstatistikAnalyzer(articles_dir)

    # Load all articles
    print("Loading and analyzing Branchenstatistik articles...")
    ...
    # Save report
    report_path = Path("/Users/fabianradlow/Documents/Code-Projekte/Operation Bona Fides/branchenstatistik_analysis_report.md")
