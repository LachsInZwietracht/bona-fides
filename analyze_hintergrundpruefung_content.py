#!/usr/bin/env python3
"""
Hintergrundprüfung Content Similarity Analysis
Analyzes 20 numbered Hintergrundprüfung articles for content duplicates
Adapted from the successful Cyberbetrug analysis
"""
...
class HintergrundpruefungAnalyzer:
    ...
    def analyze_article(self, filepath: Path) -> Dict:
        ...
        try:
            ...
            number_match = re.search(r'hintergrundpruefung-(\d+)-', filepath.name)
            article_number = int(number_match.group(1)) if number_match else 0

            return {
                'filepath': str(filepath),
                'article_number': article_number,
                'title': f"Hintergrundprüfung {article_number}" if article_number else filepath.stem,
                'sections': sections,
    ...
    def load_all_articles(self) -> Dict[str, Dict]:
        """Load and analyze all Hintergrundprüfung articles"""
        hintergrundpruefung_files = sorted(self.articles_dir.glob("*hintergrundpruefung-*-worauf-arbeitgeber-achten-sollten.md"))
        print(f"Found {len(hintergrundpruefung_files)} Hintergrundprüfung articles")

        for filepath in hintergrundpruefung_files:
    ...
    def generate_report(self, similarity_matrix: Dict, deletion_candidates: List[Dict]) -> str:
        ...
        report = []
        report.append("# Hintergrundprüfung Content Similarity Analysis Report")
...
def main():
    ...
    analyzer = HintergrundpruefungAnalyzer(articles_dir)

    # Load all articles
    print("Loading and analyzing Hintergrundprüfung articles...")
    ...
    # Save report
    report_path = Path("/Users/fabianradlow/Documents/Code-Projekte/Operation Bona Fides/hintergrundpruefung_analysis_report.md")
