#!/usr/bin/env python3
"""
Unternehmensbetrug Content Similarity Analysis
Analyzes 20 numbered Unternehmensbetrug articles for content duplicates
Adapted from the successful Cyberbetrug and Hintergrundprüfung analysis
"""
...
class UnternehmensbetrugAnalyzer:
    ...
    def analyze_article(self, filepath: Path) -> Dict:
        ...
        try:
            ...
            number_match = re.search(r'unternehmensbetrug-(\d+)-', filepath.name)
            article_number = int(number_match.group(1)) if number_match else 0

            return {
                'filepath': str(filepath),
                'article_number': article_number,
                'title': f"Unternehmensbetrug {article_number}" if article_number else filepath.stem,
                'sections': sections,
    ...
    def load_all_articles(self) -> Dict[str, Dict]:
        """Load and analyze all Unternehmensbetrug articles"""
        unternehmensbetrug_files = sorted(self.articles_dir.glob("*unternehmensbetrug-*-risiken-und-loesungen-fuer-hr.md"))
        print(f"Found {len(unternehmensbetrug_files)} Unternehmensbetrug articles")

        for filepath in unternehmensbetrug_files:
    ...
    def generate_report(self, similarity_matrix: Dict, deletion_candidates: List[Dict]) -> str:
        ...
        report = []
        report.append("# Unternehmensbetrug Content Similarity Analysis Report")
...
def main():
    ...
    analyzer = UnternehmensbetrugAnalyzer(articles_dir)

    # Load all articles
    print("Loading and analyzing Unternehmensbetrug articles...")
    ...
    # Save report
    report_path = Path("/Users/fabianradlow/Documents/Code-Projekte/Operation Bona Fides/unternehmensbetrug_analysis_report.md")
