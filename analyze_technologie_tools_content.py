#!/usr/bin/env python3
"""
Technologie & Tools Content Similarity Analysis
Analyzes 10 numbered Technologie & Tools articles for content duplicates
...
class TechnologieToolsAnalyzer:
    ...
    def analyze_article(self, filepath: Path) -> Dict:
        ...
        try:
            ...
            if 'technologie-tools-1-' in filepath.name:
                article_number = 1
            else:
                number_match = re.search(r'technologie-tools-(\d+)-', filepath.name)
                article_number = int(number_match.group(1)) if number_match else 0

            return {
                'filepath': str(filepath),
                'article_number': article_number,
                'title': f"Technologie & Tools {article_number}" if article_number else filepath.stem,
                'sections': sections,
    ...
    def load_all_articles(self) -> Dict[str, Dict]:
        """Load and analyze all Technologie & Tools articles"""
        tech_files = sorted(self.articles_dir.glob("*technologie-tools*die-wichtigsten-werkzeuge-fuer-ermittlungen.md"))
        print(f"Found {len(tech_files)} Technologie & Tools articles")
    ...
    def generate_report(self, similarity_matrix: Dict, deletion_candidates: List[Dict]) -> str:
        ...
        report = []
        report.append("# Technologie & Tools Content Similarity Analysis Report")
...
def main():
    ...
    analyzer = TechnologieToolsAnalyzer(articles_dir)

    # Load all articles
    print("Loading and analyzing Technologie & Tools articles...")
    ...
    # Save report
    report_path = Path("/Users/fabianradlow/Documents/Code-Projekte/Operation Bona Fides/technologie_tools_analysis_report.md")
