#!/usr/bin/env python3
"""
Cyberbetrug Content Similarity Analysis
Analyzes 35 numbered Cyberbetrug articles for content duplicates
"""
...
class CyberbetrugAnalyzer:
    ...
    def analyze_article(self, filepath: Path) -> Dict:
        ...
        try:
            ...
            number_match = re.search(r'cyberbetrug-(\d+)-', filepath.name)
            article_number = int(number_match.group(1)) if number_match else 0

            return {
                'filepath': str(filepath),
                'article_number': article_number,
                'title': f"Cyberbetrug {article_number}" if article_number else filepath.stem,
                'sections': sections,
    ...
    def load_all_articles(self) -> Dict[str, Dict]:
        """Load and analyze all Cyberbetrug articles"""
        cyberbetrug_files = sorted(self.articles_dir.glob("*cyberbetrug-*-neue-maschen*.md"))
        print(f"Found {len(cyberbetrug_files)} Cyberbetrug articles")

        for filepath in cyberbetrug_files:
    ...
    def generate_report(self, similarity_matrix: Dict, deletion_candidates: List[Dict]) -> str:
        ...
        report = []
        report.append("# Cyberbetrug Content Similarity Analysis Report")
...
def main():
    ...
    analyzer = CyberbetrugAnalyzer(articles_dir)

    # Load all articles
    print("Loading and analyzing Cyberbetrug articles...")
    ...
    # Save report
    report_path = Path("/Users/fabianradlow/Documents/Code-Projekte/Operation Bona Fides/cyberbetrug_analysis_report.md")
