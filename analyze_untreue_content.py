#!/usr/bin/env python3
"""
Untreue Content Similarity Analysis
Analyzes 15 numbered Untreue articles for content duplicates
...
class UntreuAnalyzer:
    ...
    def analyze_article(self, filepath: Path) -> Dict:
        ...
        try:
            ...
            # Pattern: XXX-untreue-YY-wie-man-digitale-hinweise-erkennt.md
            number_match = re.search(r'^(\d+)-untreue-', filepath.name)
            article_number = int(number_match.group(1)) if number_match else 0

            return {
                'filepath': str(filepath),
                'article_number': article_number,
                'title': f"Untreue {article_number}" if article_number else filepath.stem,
                'sections': sections,
    ...
    def load_all_articles(self) -> Dict[str, Dict]:
        """Load and analyze all Untreue articles"""
        untreue_files = sorted(self.articles_dir.glob("*untreue*wie-man-digitale-hinweise-erkennt.md"))
        print(f"Found {len(untreue_files)} Untreue articles")

        for filepath in untreue_files:
    ...
    def generate_report(self, similarity_matrix: Dict, deletion_candidates: List[Dict]) -> str:
        ...
        report = []
        report.append("# Untreue Content Similarity Analysis Report")
...
def main():
    ...
    analyzer = UntreuAnalyzer(articles_dir)

    # Load all articles
    print("Loading and analyzing Untreue articles...")
    ...
    # Save report
    report_path = Path("/Users/fabianradlow/Documents/Code-Projekte/Operation Bona Fides/untreue_analysis_report.md")
