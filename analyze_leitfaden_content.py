#!/usr/bin/env python3
"""
Leitfaden Content Similarity Analysis
Analyzes 20 numbered Leitfaden articles for content duplicates
...
class LeitfadenAnalyzer:
    ...
    def analyze_article(self, filepath: Path) -> Dict:
        ...
        try:
            ...
            if 'leitfaden-1-' in filepath.name:
                article_number = 1
            else:
                number_match = re.search(r'leitfaden-(\d+)-', filepath.name)
                article_number = int(number_match.group(1)) if number_match else 0

            return {
                'filepath': str(filepath),
                'article_number': article_number,
                'title': f"Leitfaden {article_number}" if article_number else filepath.stem,
                'sections': sections,
    ...
    def load_all_articles(self) -> Dict[str, Dict]:
        """Load and analyze all Leitfaden articles"""
        leitfaden_files = sorted(self.articles_dir.glob("*leitfaden*schritt-fuer-schritt-zur-eigenen-sicherheit.md"))
        print(f"Found {len(leitfaden_files)} Leitfaden articles")

        for filepath in leitfaden_files:
    ...
    def generate_report(self, similarity_matrix: Dict, deletion_candidates: List[Dict]) -> str:
        ...
        report = []
        report.append("# Leitfaden Content Similarity Analysis Report")
...
def main():
    ...
    analyzer = LeitfadenAnalyzer(articles_dir)

    # Load all articles
    print("Loading and analyzing Leitfaden articles...")
    ...
    # Save report
    report_path = Path("/Users/fabianradlow/Documents/Code-Projekte/Operation Bona Fides/leitfaden_analysis_report.md")
