#!/usr/bin/env python3
"""
Internationale Ermittlungen Content Similarity Analysis
Analyzes 10 numbered Internationale Ermittlungen articles for content duplicates
...
class InternationaleErmittlungenAnalyzer:
    ...
    def analyze_article(self, filepath: Path) -> Dict:
        ...
        try:
            ...
            # Extract article number from filename - handle both patterns
            if 'internationale-ermittlungen-1-' in filepath.name:
                article_number = 1
            else:
                number_match = re.search(r'internationale-ermittlungen-(\d+)-', filepath.name)
                article_number = int(number_match.group(1)) if number_match else 0

            return {
                'filepath': str(filepath),
                'article_number': article_number,
                'title': f"Internationale Ermittlungen {article_number}" if article_number else filepath.stem,
                'sections': sections,
    ...
    def load_all_articles(self) -> Dict[str, Dict]:
        """Load and analyze all Internationale Ermittlungen articles"""
        international_files = sorted(self.articles_dir.glob("*internationale-ermittlungen*grenzen-ueberschreitende-faelle-loesen.md"))
        print(f"Found {len(international_files)} Internationale Ermittlungen articles")

        for filepath in international_files:
    ...
    def generate_report(self, similarity_matrix: Dict, deletion_candidates: List[Dict]) -> str:
        ...
        report = []
        report.append("# Internationale Ermittlungen Content Similarity Analysis Report")
        ...
        if deletion_candidates:
            ...
            for candidate in sorted(deletion_candidates, key=lambda x: x['similarity_score'], reverse=True):
                report.append(
                    f"| #{candidate['delete_number']:3d} | #{candidate['keep_number']:3d} | "
...
def main():
    ...
    analyzer = InternationaleErmittlungenAnalyzer(articles_dir)

    # Load all articles
    print("Loading and analyzing Internationale Ermittlungen articles...")
    ...
    # Save report
    report_path = Path("/Users/fabianradlow/Documents/Code-Projekte/Operation Bona Fides/internationale_ermittlungen_analysis_report.md")
