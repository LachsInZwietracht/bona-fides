#!/usr/bin/env python3
"""
Häufige Fragen Content Similarity Analysis
Analyzes 10 numbered Häufige Fragen articles for content duplicates
...
class HaeufigeFragenAnalyzer:
    ...
    def analyze_article(self, filepath: Path) -> Dict:
        ...
        try:
            ...
            # Extract article number from filename - handle the specific pattern
            # Pattern: XXX-haeufige-fragen-YY-antworten-vom-privatdetektiv.md
            number_match = re.search(r'^(\d+)-haeufige-fragen-', filepath.name)
            article_number = int(number_match.group(1)) if number_match else 0

            return {
                'filepath': str(filepath),
                'article_number': article_number,
                'title': f"Häufige Fragen {article_number}" if article_number else filepath.stem,
                'sections': sections,
    ...
    def load_all_articles(self) -> Dict[str, Dict]:
        """Load and analyze all Häufige Fragen articles"""
        haeufige_fragen_files = sorted(self.articles_dir.glob("*haeufige-fragen*antworten-vom-privatdetektiv.md"))
        print(f"Found {len(haeufige_fragen_files)} Häufige Fragen articles")

        for filepath in haeufige_fragen_files:
    ...
    def generate_report(self, similarity_matrix: Dict, deletion_candidates: List[Dict]) -> str:
        ...
        report = []
        report.append("# Häufige Fragen Content Similarity Analysis Report")
        ...
        if deletion_candidates:
            ...
            report.append("## Deletion Candidates (80%+ Similarity)")
            report.append("*No articles found with 80%+ similarity.*")
            report.append("")
        ...
        # Only add content length statistics if we have data
        if len(self.articles_data) > 0:
...
def main():
    ...
    analyzer = HaeufigeFragenAnalyzer(articles_dir)

    # Load all articles
    print("Loading and analyzing Häufige Fragen articles...")
    ...
    # Save report
    report_path = Path("/Users/fabianradlow/Documents/Code-Projekte/Operation Bona Fides/haeufige_fragen_analysis_report.md")
